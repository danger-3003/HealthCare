...
const cards = (props:any) => {
    return (
        <View>
            <View className={`${props.bg} h-28 w-28 mx-2 px-3 py-5 pb-3 flex items-center justify-end rounded-3xl relative shadow-md shadow-slate-600`}>
                <Text className="absolute text-slate-900 font-semibold left-2.5 top-2">Avg</Text>
                <Text className={`${props.color} font-bold mt-2`} style={{fontSize:Number(props.size)}}>{props.text}</Text>
